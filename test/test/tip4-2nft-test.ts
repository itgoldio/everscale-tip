...
let nft: Contract<FactorySource["TIP4_2NftCon"]>;
let Nft;
let accountsFactory;
let manager;
...
describe("Test TIP4_2Nft contract", async function () {
  before(async () => {
  ...
  describe("Contracts", async function () {
    it("Load contract factory", async function () {
      const nftData = await locklift.factory.getContractArtifacts("TIP4_2NftCon");
    ...
    it("Deploy contract", async function () {
      await collection.methods.mintNft({id: 0, owner: manager.address, json: nftJSON}).sendExternal({ publicKey: signer.publicKey });
    ...
    it("Transfer test with wrong owner", async function () {
      ...
      it("Get JSON test", async function () {
        let response = await nft.methods.getJson({answerId: 0}).call();
        expect(response.json).to.be.equal(nftJSON);
      });
    ...
    //   expect(await locklift.provider.getBalance(callbackReceiver.address).then(balance => Number(locklift.utils.toNano(balance)))).to.be.above(0);
